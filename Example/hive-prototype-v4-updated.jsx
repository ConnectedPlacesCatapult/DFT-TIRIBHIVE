  ...
  return (
    <div className="relative py-6" style={{ overflow: "visible" }}>
      <div style={{ overflow: "hidden", paddingTop: "12px", paddingBottom: "12px", marginBottom: "4px" }} onMouseEnter={e => e.currentTarget.querySelector('.track-a').style.animationPlayState = 'paused'} onMouseLeave={e => e.currentTarget.querySelector('.track-a').style.animationPlayState = 'running'}>
        <div className="track-a flex gap-3" style={{ width: "max-content", animation: "scrollX 130s linear infinite" }}>
      ...
      <div style={{ overflow: "hidden", paddingTop: "12px", paddingBottom: "12px" }} onMouseEnter={e => e.currentTarget.querySelector('.track-b').style.animationPlayState = 'paused'} onMouseLeave={e => e.currentTarget.querySelector('.track-b').style.animationPlayState = 'running'}>
        <div className="track-b flex gap-3" style={{ width: "max-content", animation: "scrollX 155s linear infinite reverse" }}>
...
// ── 3D MARQUEE ──
const Marquee3D = ({ cases, onCardClick, matchingSectors, matchingHazards, hasFilters, gradFade = '#0d1117' }) => {
  const cols = [[], [], [], [], [], []];
  ...
  return (
    <div className="relative overflow-hidden" style={{ height: "500px" }}>
      <div className="absolute inset-0 flex items-center justify-center" style={{ perspective: "900px" }}>
        <div className="flex gap-4" style={{ transform: "translateX(-60px) translateZ(-100px) rotateX(14deg) rotateY(-6deg) rotateZ(8deg)" }}>
          {cols.map((colCases, colIdx) => (
            <div key={colIdx} className="overflow-hidden flex-shrink-0" style={{ width: "224px", height: "440px" }}
              onMouseEnter={e => e.currentTarget.querySelector('.col-inner').style.animationPlayState = 'paused'}
              onMouseLeave={e => e.currentTarget.querySelector('.col-inner').style.animationPlayState = 'running'}>
              <div className="col-inner flex flex-col gap-3 p-2"
                style={{ animation: `scrollY ${colIdx % 2 === 0 ? 30 : 38}s linear infinite ${colIdx % 2 === 1 ? "reverse" : ""}` }}>
                {[...colCases, ...colCases].map((c, i) => <MarqueeCard key={`col${colIdx}-${i}`} c={c} onClick={onCardClick} dimmed={isDimmed(c)} highlighted={isHighlighted(c)} />)}
              </div>
            </div>
          ))}
        </div>
      </div>
      <div className="pointer-events-none absolute inset-x-0 top-0 h-24" style={{ background: `linear-gradient(180deg, ${gradFade} 0%, transparent 100%)` }} />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24" style={{ background: `linear-gradient(0deg, ${gradFade} 0%, transparent 100%)` }} />
    </div>
  );
};
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .hive-root .border-emerald-100, .hive-root .border-emerald-200 { border-color: var(--accent) !important; opacity: 0.4; }
      `}</style>
        ...
        <div className="border-t pt-8 pb-4" style={{ borderColor: 'var(--border)' }}>
          <div className="max-w-6xl mx-auto px-6 mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 rounded-full p-0.5 border" style={{ borderColor: 'var(--border)', background: 'var(--surface-alt)' }}>
              {["2d", "3d"].map(v => (
                <button key={v} onClick={() => setMarqueeView(v)}
                  className="text-xs font-semibold px-3 py-1.5 rounded-full transition-all uppercase tracking-wider"
                  style={{ background: marqueeView === v ? 'var(--text-primary)' : 'transparent', color: marqueeView === v ? 'var(--surface)' : 'var(--text-secondary)' }}>
                  {v}
                </button>
              ))}
            </div>
          ...
          {marqueeView === "2d"
            ...
            : <Marquee3D cases={MARQUEE_CASES} onCardClick={handleMarqueeCardClick} matchingSectors={marqueeMatchingSectors} matchingHazards={marqueeMatchingHazards} hasFilters={marqueeHasFilters} gradFade={T.gradFade} />
          }
